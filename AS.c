...
bool get_auction_owner_UID(int auctionID, char* ownerUID) {
    char filename[50];
    snprintf(filename, sizeof(filename), "AUCTION/%d/START_%d", auctionID, auctionID);
...
void bid_command(char* buffer, int new_fd, int nread){
    char UID[UID_LEN+1];
    char UID_login_file_path[UID_LOGIN_FILE_LEN+1];
    char password[PW_LEN+1];
    int value;
    ...
    sscanf(buffer, "BID %s %s %d %d", UID, password, &AID,&value);
    printf("Received Data: UID=%s, password=%s, AID=%d, value=%d\n", UID, password, AID, value);
    printf("ok1\n");
    /*if (!check_UID_format(UID)) {
    ...
    printf("ok3\n");
    snprintf(pathname, sizeof(pathname), "AUCTIONS/%s", AID);
